...
export default({
    ...
    methods: {
         del: async function(index){
            console.log(index);
            await axios.post("http://localhost:3030/delSupplier", index);
        ...
        save: async function(){
            try {
                var form = this.form;
                form["supplier_id"] = this.currentID;
                for (const prop in form)
                    if(form[prop] == "") return window.alert("All fields must be filled");

                console.log(JSON.stringify(form));
                var req = await axios.post("http://localhost:3030/updSupplier", JSON.stringify(form));
        ...
        submit: async function(){
            try{
                ...
                for (const prop in form)
                    if(form[prop] == "") return window.alert("All fields must be filled");
                console.log(JSON.stringify(form));
                await axios.post("http://localhost:3030/insSupplier", JSON.stringify(form));
...
async function getData(){
    try {
        var res = await axios.get("http://localhost:3030/suppliers");
...
<div>
    ...
    <h1>Suppliers&nbsp;<span class="fa fa-university" /></h1>
    <Datatable v-if="flag">
        ...
        <template v-slot:tbody>
            <tr v-for="(obj, index) in tableData" :key="index">
                ...
                <td>{{obj.zip}}</td>
                <td>
                    <span class="fa fa-pencil" style="color: #00a100; margin-right: 2rem; cursor: pointer;" @click="updShow(obj.supplier_id)"/>
                    <span class="fa fa-minus-circle" style="color: #a10000; cursor: pointer;" @click="del(obj.supplier_id)"/>
                </td>
            </tr>
        </template>
    </Datatable>
    ...
    <Modal id="insModal" :show="this.showIns" @close="this.insToggle()" @save="this.submit()">
        <template v-slot:header><h1>Add a Supplier</h1></template>
